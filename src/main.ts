...
async function main() {
  ...
  const refreshButtonElClickHandler = () => {
    ...
    if (keyword) {
      if (
        keyword.indexOf("https://") === 0 ||
        keyword.indexOf("http://") === 0 ||
        keyword.indexOf("file://") === 0
      ) {
        url = keyword;
      } else {
  ...
  const openModel = {
    async show() {
      pass = "";
      messageEl &&
        (messageEl.innerText = "Set your unlock pattern to lock screen.");
      lock.clear();
      if (refreshButtonEl) {
        refreshButtonEl.removeEventListener(
          "click",
          refreshButtonElClickHandler,
          false
        );
        refreshButtonEl.addEventListener(
          "click",
          refreshButtonElClickHandler,
          false
        );
  ...
  keywordInputEl.removeEventListener(
    "keyup",
    keywordInputElEnterHandler,
    false
  );
    ...
    lock
      ...
      .on("complete", async (pattern) => {
        if (!pass) {
          pass = pattern;
          messageEl &&
            (messageEl.innerText = "Use your pattern to unlock screen.");
          lock.clear();
